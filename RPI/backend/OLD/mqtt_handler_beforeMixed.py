...
class WineFridgeController:
    ...
    def on_connect(self, client, userdata, flags, rc, properties):
        ...
        # Subscribe to relevant topics (done on every connect/reconnect)
    ...
    def start_bottle_load(self, data):
        ...
        # Find empty position across functional drawers (3, 5, 7)
        drawer_id, position = self.find_empty_position()

        if not drawer_id or not position:
            print("[ERROR] No empty positions in functional drawers")
            error_msg = {
                "action": "load_error",
                "source": "mqtt_handler",
                "data": {
        ...
        print(f"[LOAD] Found empty position: {drawer_id}, position {position}")
    ...
    def start_bottle_unload(self, data):
        ...
        # Validate that drawer is functional (has sensors)
        if not self.is_functional_drawer(drawer_id):
            print(f"[ERROR] Drawer {drawer_id} is not equipped with sensors")
            error_msg = {
                "action": "unload_error",
                "source": "mqtt_handler",
                "data": {
                    "error": f"Drawer {drawer_id} is not equipped with sensors. Use drawers 3, 5, or 7."
                },
                "timestamp": datetime.now().isoformat()
            }
            self.client.publish("winefridge/system/status", json.dumps(error_msg))
        ...
        self.client.publish(f"winefridge/{drawer_id}/command", json.dumps(led_cmd))
        print(f"[UNLOAD] LED command sent")

        # 2. Notify web to show the position
        web_msg = {
    ...
    def is_functional_drawer(self, drawer_id):
        """Check if drawer has sensors for bottle operations"""
        return drawer_id in FUNCTIONAL_DRAWERS
    ...
    def find_empty_position(self):
        """Find first empty position across all functional drawers (3, 5, 7)"""
        ...
        return None, None  # No empty positions
    ...
    def run(self):
        print("[START] WineFridge MQTT Handler v2.3 (LOAD+UNLOAD Multi-Drawer)")
        # loop_forever() is blocking and handles reconnection automatically
